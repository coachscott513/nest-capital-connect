...
import { saratogaData } from "@/data/townData";

const SaratogaHomesForSalePage = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const iframe = document.getElementById("saratoga-listings-iframe") as HTMLIFrameElement;
      if (iframe) {
        try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("leads").insert({
        ...
        message: formData.requirements || "Saratoga Springs home search request", type: "saratoga-homes",
        origin_town: "saratoga-springs", lead_type: "buyer",
      });
  ...
  return (
    <>
      <Helmet>
        <title>Saratoga Springs Homes for Sale | Capital District Nest</title>
        <meta name="description" content="Browse live Saratoga Springs homes for sale plus buyer tools and local market insights." />
        <link rel="canonical" href="https://capitaldistrictnest.com/saratoga-springs-homes-for-sale" />
      </Helmet>
      ...
      <div className="min-h-screen bg-background">
        ...
        <section className="py-14 bg-background">
          <div className="max-w-3xl mx-auto px-6">
            <h2 className="text-3xl font-bold text-foreground mb-8 text-center">Why Saratoga Springs?</h2>
            <ul className="space-y-4 text-lg text-foreground">
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>World-famous horse racing and vibrant downtown</span></li>
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Excellent Saratoga Springs City School District</span></li>
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Historic architecture and charming neighborhoods</span></li>
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Strong appreciation and consistent demand</span></li>
            </ul>
          </div>
        </section>
...
export default SaratogaHomesForSalePage;