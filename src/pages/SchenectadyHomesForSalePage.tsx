...
import { schenectadyData } from "@/data/townData";

const SchenectadyHomesForSalePage = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const iframe = document.getElementById("schenectady-listings-iframe") as HTMLIFrameElement;
      if (iframe) {
        try {
  ...
  return (
    <>
      <Helmet>
        <title>Schenectady Homes for Sale | Capital District Nest</title>
        <meta name="description" content="Browse live Schenectady homes for sale plus buyer tools and local market insights." />
        <link rel="canonical" href="https://capitaldistrictnest.com/schenectady-homes-for-sale" />
      </Helmet>
      ...
      <div className="min-h-screen bg-background">
        ...
        <section className="pt-28 pb-16 bg-gradient-to-br from-primary/10 to-primary/5">
          <div className="max-w-4xl mx-auto px-6 text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">Schenectady Homes for Sale</h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="bg-primary hover:bg-primary/90 font-semibold px-8 py-6 text-lg" asChild>
                <a href={schenectadyData.remaxSearchUrl} target="_blank" rel="noopener noreferrer">View Live Schenectady Listings <ExternalLink className="w-5 h-5 ml-2" /></a>
              </Button>
        ...
        <section className="py-14 bg-muted/30">
          <div className="max-w-5xl mx-auto px-6">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-foreground mb-3">Live Schenectady Listings (RE/MAX)</h2>
            </div>
            {!iframeError ? (
              <div className="w-full max-w-[1200px] mx-auto rounded-[16px] overflow-hidden border-2 border-primary/30 bg-white">
                <iframe id="schenectady-listings-iframe" title="Schenectady Home Search" src={schenectadyData.remaxSearchUrl} loading="lazy" className="w-full h-[800px] md:h-[1100px] block border-0" onError={() => setIframeError(true)} />
              </div>
            ) : (
              <Card className="max-w-2xl mx-auto p-8 text-center">
                <Home className="w-8 h-8 text-primary mx-auto mb-4" />
                <h3 className="text-2xl font-bold mb-3">View Schenectady Listings</h3>
                <Button size="lg" asChild><a href={schenectadyData.remaxSearchUrl} target="_blank" rel="noopener noreferrer">Open RE/MAX Search</a></Button>
              </Card>
            )}
          </div>
        </section>
        ...
        <section className="py-14 bg-background">
          <div className="max-w-3xl mx-auto px-6">
            <h2 className="text-3xl font-bold text-foreground mb-8 text-center">Why Schenectady?</h2>
            <ul className="space-y-4 text-lg text-foreground">
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Affordable entry points with strong cash flow potential</span></li>
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Historic Stockade District with colonial architecture</span></li>
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Growing downtown with Proctors Theatre and dining</span></li>
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Close to Union College and major employers</span></li>
            </ul>
          </div>
        </section>
...
export default SchenectadyHomesForSalePage;