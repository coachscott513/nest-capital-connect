    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="pt-32 pb-8 px-6">
        <div className="max-w-5xl mx-auto">
          <Link to="/analyze" className="inline-flex items-center gap-2 text-muted-foreground/70 hover:text-foreground transition-colors mb-8 text-sm font-medium">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl">Review HOA, insurance, special assessments, and total monthly ownership cost.</p>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-6 p-8 rounded-3xl border border-border/50 bg-secondary/50">
            ...
            {[
              ...
              <div key={field.id}>
                <Label className="text-foreground/80 text-sm font-medium">{field.label}</Label>
                <Input
                  type="number"
                  value={field.val}
                  step={field.step}
                  onChange={(e) => setInputs({ ...inputs, [field.id]: parseFloat(e.target.value) || 0 })}
                  className="mt-1 bg-background border-border text-foreground rounded-xl h-12"
                />
              </div>
            ))}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="p-8 rounded-3xl bg-foreground text-background text-center">
              ...
              <p className="text-muted-foreground/70 text-sm mt-2">all-in monthly ownership cost</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-6 rounded-2xl bg-secondary/40 border border-border/50 text-center">
                ...
                <p className="text-2xl font-bold text-foreground">${fmt(cashToClose)}</p>
              </div>
            </div>
          </div>
        </div>
