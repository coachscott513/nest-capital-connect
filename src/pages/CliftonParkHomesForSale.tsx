...
import { cliftonParkData } from "@/data/townData";

const CliftonParkHomesForSale = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const iframe = document.getElementById("cliftonpark-listings-iframe") as HTMLIFrameElement;
      if (iframe) {
        try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("leads").insert({
        ...
        message: formData.requirements || "Clifton Park home search request", type: "clifton-park-homes",
        origin_town: "clifton-park", lead_type: "buyer",
      });
  ...
  return (
    <>
      <Helmet>
        <title>Clifton Park Homes for Sale | Capital District Nest</title>
        <meta name="description" content="Browse live Clifton Park homes for sale plus buyer tools and local market insights." />
        <link rel="canonical" href="https://capitaldistrictnest.com/clifton-park-homes-for-sale" />
      </Helmet>
      ...
      <div className="min-h-screen bg-background">
        ...
        <section className="pt-28 pb-16 bg-gradient-to-br from-primary/10 to-primary/5">
          <div className="max-w-4xl mx-auto px-6 text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">Clifton Park Homes for Sale</h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="bg-primary hover:bg-primary/90 font-semibold px-8 py-6 text-lg" asChild>
                <a href={cliftonParkData.searchPath} target="_blank" rel="noopener noreferrer">View Live Clifton Park Listings <ExternalLink className="w-5 h-5 ml-2" /></a>
              </Button>
        ...
        <section className="py-14 bg-muted/30">
          <div className="max-w-5xl mx-auto px-6">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-foreground mb-3">Live Clifton Park Listings</h2>
            </div>
            {!iframeError ? (
              <div className="w-full max-w-[1200px] mx-auto rounded-[16px] overflow-hidden border-2 border-primary/30 bg-background">
                <iframe id="cliftonpark-listings-iframe" title="Clifton Park Home Search" src={cliftonParkData.searchPath} loading="lazy" className="w-full h-[800px] md:h-[1100px] block border-0" onError={() => setIframeError(true)} />
              </div>
            ) : (
              <Card className="max-w-2xl mx-auto p-8 text-center">
                <Home className="w-8 h-8 text-primary mx-auto mb-4" />
                <h3 className="text-2xl font-bold mb-3">View Clifton Park Listings</h3>
                <Button size="lg" asChild><a href={cliftonParkData.searchPath} target="_blank" rel="noopener noreferrer">Search Homes</a></Button>
              </Card>
            )}
          </div>
        </section>
        ...
        <section className="py-14 bg-background">
          <div className="max-w-3xl mx-auto px-6">
            <h2 className="text-3xl font-bold text-foreground mb-8 text-center">Why Clifton Park?</h2>
            <ul className="space-y-4 text-lg text-foreground">
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Top-rated Shenendehowa Central School District</span></li>
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Excellent highway access to Albany and Saratoga</span></li>
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Modern housing stock and planned communities</span></li>
              <li className="flex items-start gap-3"><Check className="w-5 h-5 text-primary mt-1" /><span>Tech corridor employers and strong job market</span></li>
            </ul>
          </div>
        </section>
...
export default CliftonParkHomesForSale;